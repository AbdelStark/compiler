...
#[test]
fn introspector_full_corpus_delta_matrix_kind_parity() {
    ...
    for vector in vectors {
        ...
        if !output.status.success() {
            mismatch_names.push(vector.name.clone());
        ...
        let parsed: ExternalParityResult = match serde_json::from_slice(&output.stdout) {
            ...
            Err(err) => {
                mismatch_names.push(vector.name.clone());
                mismatches.push(format!(
                    "{} invalid adapter json output: {} (stdout={})",
                    vector.name,
                    err,
    ...
    mismatch_names.sort();
    mismatch_names.dedup();

    let mut expected = KNOWN_INTROSPECTOR_DELTAS
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    expected.sort();

    if mismatch_names != expected {
        panic!(
            "introspector full-corpus delta set changed\nexpected={:?}\nactual={:?}\ndetails:\n{}",
            expected,
            mismatch_names,
            mismatches.join("\n")
        );
